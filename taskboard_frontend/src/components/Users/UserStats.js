...
const UserStats = ({ boardId }) => {
  ...
  const getCardsOfBoard = async () => {
    ...
    const cardPermissions = await axios.get(`http://127.0.0.1:3001/cardPermission/${params.member}`);
    cardPermissions.data.map((cp) => cardPermissionsCards.push(cp.card));
    
    const listsOfBoard = await axios.post(`http://127.0.0.1:3001/list/board/${boardId}`, { cards: cardPermissionsCards });
