...
const AddLabel = ({ card, setCardLabel, boardLists, setBoardLists, list }) => {
  ...
  const labelsOfBoard = async () => {
    const boardLabels = await axios.get(`http://127.0.0.1:3001/label/${list.board_id}`);
  ...
  const handleNewLabel = async () => {
    ...
    const nl = await axios.post('http://127.0.0.1:3001/label', { ...newLabel, board: list.board_id });
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      // await axios.patch(`http://127.0.0.1:3001/label/${selectedLabel._id}`, { card: card._id });
      await axios.patch(`http://127.0.0.1:3001/card/${card._id}`, { label: selectedLabel._id });
      const newList = {
        ...list,
