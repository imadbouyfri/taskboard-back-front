...
const AddCard = ({ toggleAddCard, setToggleAddCard, list, boardLists, setBoardLists, setList }) => {
  ...
  const handleOnClick = async (e) => {
    ...
    const { data } = await axios.post(
      `http://197.153.57.185:3001/card/create`,
      {
        name: cardTitle,
        list_id: list._id
      },
    ...
    await axios.post('http://197.153.57.185:3001/cardPermission', { card: data._id, user: user._id, role: 'admin' });
    const cardResponse = await axios.get(`http://197.153.57.185:3001/card/${data._id}`);
    const newCard = {
    ...
    setCardTitle("");
    // add card to history
    await axios.post("http://197.153.57.185:3001/cardHistory", {
      user: user._id,
      card: newCard._id,
      board: list.board_id,
      action: `add`,
