...
export default function CardMemberInvite({ notCardMembers, cardMembers, setCardMembers, card, boardLists, setBoardLists, list }) {
  ...
  const handleOnSelect = async (mem) => {
    ...
    await axios.post('http://197.153.57.185:3001/cardPermission', {user: mem._id, card: card._id, role: 'invited'});
    const boardId = list.board_id;
    console.log(boardId);
    const boardResponse = await axios.get(`http://197.153.57.185:3001/board/${boardId}`);
    ...
    console.log(data1);
    await axios.post("http://197.153.57.185:3001/notification", data1);
    const newCard = await axios.get(`http://197.153.57.185:3001/card/${card._id}`);
