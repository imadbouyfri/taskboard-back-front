...
const Board = () => {
  ...
  const getSingleBoard = async () => {
    ...
    try {
      const response = await axios.get(process.env.API_URL+`/board/${id}`, config);
  ...
  const updateLists = async (source, destination) => {
    ...
    if (sourceList._id === destinationList._id) {
      try {
        ...
        await axios.patch(process.env.API_URL+`/list/${sourceList._id}`, {
          cards: cards,
        });
        getSingleBoard();
      } catch (err) {
      ...
      try {
        ...
        if (destinationList.name.toLowerCase() === 'done') {
          await axios.patch(process.env.API_URL+`/card/${draggedCard._id}`, {
        ...
        await axios.patch(process.env.API_URL+`/list/${destinationList._id}`, {
  ...
  const updateBoard = async (source, destination) => {
    ...
    try {
      await axios.patch(process.env.API_URL+`/board/${id}`, {
        lists: newBoardLists,
      }, config);
    } catch (err) {
      console.log(err);
  ...
  const getAllMembers = async () => {
    ...
    try {
      // get invited members
      const response2 = await axios.get(process.env.API_URL+`/member/${id}`);
      ...
      // get All members
      const response1 = await axios.get(process.env.API_URL+"/member", config);
