...
const Board = () => {
  ...
  const getSingleBoard = async () => {
    ...
    try {
      const response = await axios.get(`http://127.0.0.1:3001/board/${id}`, config);
  ...
  const updateLists = async (source, destination) => {
    ...
    if (sourceList._id === destinationList._id) {
      try {
        ...
        await axios.patch(`http://127.0.0.1:3001/list/${sourceList._id}`, {
          cards: cards,
        });
        getSingleBoard();
      } catch (err) {
      ...
      try {
        ...
        if (destinationList.name.toLowerCase() === 'done') {
          await axios.patch(`http://127.0.0.1:3001/card/${draggedCard._id}`, {
        ...
        await axios.patch(`http://127.0.0.1:3001/list/${destinationList._id}`, {
  ...
  const updateBoard = async (source, destination) => {
    ...
    try {
      await axios.patch(`http://127.0.0.1:3001/board/${id}`, {
        lists: newBoardLists,
      }, config);
    } catch (err) {
      console.log(err);
  ...
  const getAllMembers = async () => {
    ...
    try {
      // get invited members
      const response2 = await axios.get(`http://127.0.0.1:3001/member/${id}`);
      ...
      // get All members
      const response1 = await axios.get("http://127.0.0.1:3001/member", config);
